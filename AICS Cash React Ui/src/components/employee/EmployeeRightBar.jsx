...
export default function EmployeeRightBar() {
  ...
  //   let withdraws = []
  //   withdraws = transactions.filter(tr => tr.employee.matricle===JSON.parse(localStorage.getItem("user")).matricle)
  ...
  return (
    <>
        <div className='w-full px-4 py-2 h-full bg-gray-50'>
            ...
            <div className='text-center mt-7 font-semibold text-teal-700 text-[30px]'>Recent Transactions</div>
            <div className='flex flex-col justify-between mt-7 h-[60vh] items-center'>
              {/* {
                [...transactions].map(transaction => (
                    <div onClick={() => handleOpenModal(transaction)} className={`${transaction.transaction.transaction_type==="DEPOSIT"?"bg-green-500":"bg-red-500"} hover:bg-opacity-70 text-[17px] text-white cursor-pointer p-4 w-full rounded-[10px] flex flex-row justify-between items-center`}>
                        ...
                        <div className='font-bold'>{transaction.transaction.transaction_type}</div>
                        <div className='font-bold'>{new Date(transaction.transaction.transaction_date).toLocaleDateString()}</div>
                    </div>
                ))
              } */}
              {
                [...DUMMYTRANSACTIONS].map(transaction => (
                    <div onClick={() => handleOpenModal(transaction)} className={`${transaction.transaction_type==="Deposit"?"bg-green-500":"bg-red-500"} hover:bg-opacity-70 text-[17px] text-white cursor-pointer p-4 w-full rounded-[10px] flex flex-row justify-between items-center`}>
                        <div className='font-bold'>
                          <Avatar sx={{backgroundColor: "white", color:"black", textTransform: "uppercase"}}>{transaction.receiver[0]}{transaction.receiver[1]}</Avatar>
                        </div>
                        <div className='font-bold'>{transaction.transaction_type}</div>
                        <div className='font-bold'>{new Date(transaction.date).toLocaleDateString()}</div>
                    </div>
                ))
              }
            </div>
      ...
      >
        ...
        <DialogTitle className='text-teal-800 text-center text-[36px] font-bold' sx={{fontSize: "28px", fontWeight: 700}}>Transaction Information</DialogTitle>
              <DialogContent>
